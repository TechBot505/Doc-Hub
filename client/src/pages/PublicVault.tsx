...
import { Pdf, usePdfContext } from "../context/PdfContext";
...
function PublicVault() {
  ...
  const handleOnClick = () => {
    const findUsers = allPdfs.filter((pdf) => pdf.name.includes(searchText));
    if (searchText === "") {
      return setAllPdfs(allPdfs);
    }
    setAllPdfs(findUsers);
  }
